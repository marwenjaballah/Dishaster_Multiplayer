...
public class CrisisDebugUI : MonoBehaviour {
    ...
    private void OnGUI() {
        ...
        GUILayout.Label($"Fires: {(activeFires > 0 ? $"<color=orange>{activeFires} ACTIVE</color>" : "0")}", labelStyle);
        ...
        if (GUILayout.Button($"AUTO CRISIS: {(isAuto ? "ON" : "OFF")}", buttonStyle, GUILayout.Height(24))) {
